...
class TTTMLP(TTTBase):
    def __init__(self, config: TTTConfig, layer_idx: Optional[int] = None):
        ...
        self.moe = DenseMoE(dim=self.head_dim, num_experts=4)
    ...
    def ttt(
        ...
        def compute_mini_batch(params_dict, inputs):
            ...
            XK_mini_batch = self.moe(XK_mini_batch)
...
class FeedForward(nn.Module):
    def __init__(self, dim, hidden_dim):
        super().__init__()
        self.w1 = nn.Linear(dim, hidden_dim) # gate path
        self.w2 = nn.Linear(dim, hidden_dim) # value path
    ...
    def init_weights(self):
        ...
        # Optional: scale output layer to prevent signal from being too strong
        self.w3.weight.data *= 0.70710678118  # approx 1/sqrt(2)
...
class DenseMoE(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.experts = nn.ModuleList(
            [
                FeedForward(dim=dim, hidden_dim=dim*4, *args, **kwargs)
    ...
    def forward(self, x: Tensor):
        ...
        # 1. Flatten for routing
        # [B * nh * k, f]
        x_flat = x.reshape(B * nh * k, f)

        # 2. Routing decision
        # (batch_size, seq_len, num_experts)
        router_weights = self.gate(x_flat)
        ...
        for expert_idx, expert in enumerate(self.experts):
            ...
            combined_output += expert_output * router_weights[:, expert_idx].unsqueeze(-1)

        return combined_output.reshape(B, nh, k, f)
